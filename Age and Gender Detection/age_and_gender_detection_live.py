...
def predict_age_and_gender():
    ...
    while True:
        ...
        for i, (start_x, start_y, end_x, end_y) in enumerate(faces):
            ...
            cv2.putText(frame, label, (start_x, yPos), cv2.FONT_HERSHEY_SIMPLEX, 0.54, box_color, 2)
        
        cv2.imshow("Gender Estimator", frame)
        
        if cv2.waitKey(1) == ord("q") or cv2.getWindowProperty("Gender Estimator", cv2.WND_PROP_VISIBLE) < 1:
            break
